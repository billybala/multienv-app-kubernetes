...
function revert_backend() {
  ...
  kubectl set image deployment/${BACKEND_DEPLOYMENT} backend=${BACKEND_OLD_IMAGE} -n ${NAMESPACE}
  kubectl scale deployment/${BACKEND_DEPLOYMENT} --replicas=${REPLICAS_PRODUCTION} -n ${NAMESPACE}
...
function revert_frontend() {
  ...
  kubectl set image deployment/${FRONTEND_DEPLOYMENT} frontend=${FRONTEND_OLD_IMAGE} -n ${NAMESPACE}
  kubectl scale deployment/${FRONTEND_DEPLOYMENT} --replicas=${REPLICAS_PRODUCTION} -n ${NAMESPACE}
...
kubectl scale deployment/${BACKEND_DEPLOYMENT} --replicas=1 -n ${NAMESPACE}
kubectl rollout status deployment/${BACKEND_DEPLOYMENT} -n ${NAMESPACE} --timeout=120s
...
kubectl scale deployment/${FRONTEND_DEPLOYMENT} --replicas=1 -n ${NAMESPACE}
kubectl rollout status deployment/${FRONTEND_DEPLOYMENT} -n ${NAMESPACE} --timeout=120s
